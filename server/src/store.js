 ...
 */
export function getAvailable({ search = '', offset = 0, limit = 20 } = {}) {
  const items = [];
  let total = 0;

  // Fast path for total when there is no search filter.
  const fastTotal = search ? null : universeSize() - selectedOrder.length;

  forEachAscending((id) => {
    if (selectedSet.has(id)) return;
    if (!matchesSearch(id, search)) return;
    if (total >= offset && items.length < limit) {
      items.push(id);
    }
    total++;
    // If we already filled the page and we don't need an exact total, stop.
    if (fastTotal !== null && items.length >= limit) return false;
    return true;
  });

  const resolvedTotal = fastTotal !== null ? fastTotal : total;
  return {
    items,
    total: resolvedTotal,
    offset,
 ...
 * Returns a page { items, total, offset, limit, hasMore }.
 */
export function getSelected({ search = '', offset = 0, limit = 20 } = {}) {
