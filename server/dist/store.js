...
function countAvailable(search) {
    if (!search)
        return universeSize() - selectedOrder.length;
...
export function select(id) {
    if (!Number.isInteger(id) || !existsInUniverse(id))
        return false;
    if (selectedSet.has(id))
        return false;
    selectedSet.add(id);
...
export function setOrder(order) {
    if (!Array.isArray(order))
        return false;
    if (order.length !== selectedOrder.length)
        return false;
    const seen = new Set();
